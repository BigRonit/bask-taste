...
def supadata_metadata(url):
    """Returns platform stats: views, likes, comments, shares.
    Shares-per-view is the send-rate proxy — the variable that separates
    'well watched' from 'blew up'."""
    endpoint = "https://api.supadata.ai/v1/metadata?" + urllib.parse.urlencode(
        {"url": url})
    data = json.loads(fetch(endpoint, {"x-api-key": SUPADATA_KEY}))
...
def flags(m):
    ...
    if d and not (15 <= d <= 90):
        out.append(f"duration {d}s outside 15-90s")
    if wps and not (2.5 <= wps <= 4.2):
        out.append(f"pace {wps} w/s outside yap band")
